...
class Issuer extends Module {
  ...
  // 第一条为跳转指令时，强制与延迟槽指令一起发射
  io.out.issue_count := Mux((has_raw_hazard || has_device_hazard || has_ram_access ||
    io.in_decoder1.is_privileged || io.in_decoder2.is_privileged || is_decoder2_jump || !io.in_decoder2.is_valid) &&
    !io.in_decoder1.is_jump, 1.U, 2.U)
