...
class InvalidateQueue(config:CacheConfig) extends Module {
  ...
  for(i<- 0 to 1) {
    switch(state(i)) {
      ...
      is(sTrans) {
        when(io.writeData.ready && io.writeAddr.bits.id === worker_id(i)) {
