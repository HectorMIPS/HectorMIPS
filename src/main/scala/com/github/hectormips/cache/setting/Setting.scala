 ...
 *
 * @param _wayNum                   路数
 * @param WaySize                  路大小，大于4KB需要支持重命名寄存器
 * @param DataWidthByByte           数据宽度
 * @param victimDepth               victim buffer的深度，太深容易影响时序
 * @param victim_fetch_cycles       victim 取数据/写数据周期数
 */
class CacheConfig(val _wayNum: Int = 2, val WaySize: Int = 4 * 1024, val DataWidthByByte: Int = 16,
  ...
  // 地址划分
  val tagWidth = 32 - log2Ceil(WaySize)
  val offsetWidth = log2Ceil(DataWidthByByte) //4
  val indexWidth = log2Ceil(WaySize) - offsetWidth //路大小位数 减去offset位数,8
  val wayWidth = log2Ceil(WaySize)
